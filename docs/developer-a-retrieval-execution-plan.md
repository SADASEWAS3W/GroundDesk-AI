...
1. 先固定公共接口，再实现具体算法。
2. 各阶段保持可独立测试与回退的边界，但同一任务不要求每阶段执行测试或提交；全部实现完成后统一验证一次，提交仅在用户明确要求时进行。
...
验证时机统一遵循 `.agent-harness/rules/verification-rules.md`。下文每阶段的测试与验收条目是整项任务最终验证的输入，不是进入下一阶段前必须重复执行的门禁；基线版本和数据可提前冻结，基线与候选效果对比集中在最终验证轮次完成。
...
时间是建议值。阶段结束记录实现进度及待验证项，可以继续下一阶段，不要求先跑测试或提交。全部阶段的实现、测试代码和文档完成后，统一执行一次去重后的验收计划；出现失败时修复并补跑失败项与受影响项，未通过的内容不能标记为已验收。只有用户明确要求独立阶段交付时，才提前验证该交付边界。
...
4. 实现最小功能；
5. 增加单元测试；
6. 将相关测试、静态检查和专项验收加入任务的待验证清单；
7. 展示修改摘要，明确标记“待最终验证”；
8. 继续下一阶段，不因阶段切换自动运行门禁或提交。

所有阶段完成后统一收尾：

1. 完成使用文档，合并并去重所有阶段的验收要求；
2. 执行一轮最终测试、静态检查及必要的已授权评测；
3. 如有失败，修复并仅补验失败项和受影响项；
4. 汇总实际结果、未执行项及风险，交付任务；
5. 仅在用户明确要求时提交，不因提交重复已有且仍有效的验证。
...
正式编码时先执行“阶段一：现状盘点与基线冻结”。这一步只盘点代码、配置和基线版本，不要求运行现有检索，也不重构、不增加依赖、不修改公共接口。基线的实际评测与候选对比安排在最终验证中。完成后先输出：

- 当前检索调用链；
- 当前数据库与 Embedding 契约；
- 当前 Vector-only 基线版本与配置（未执行时标记待最终验证）；
- 下一阶段预计新增和修改的文件。
